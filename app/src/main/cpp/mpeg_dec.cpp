...
static StreamBuffer CRC_stream;
static unsigned char *g_pMp2AudioFrame;
static unsigned char *CRC_frame;
static AudioHeader audio_header;
static F_PAD f_pad;
static TEXT text;
static DynamicLabel d_label;
static XPADS_SETTING g_sXpad;
static XPADS_SETTING *g_pXpad;

static TEXT Label;
static unsigned int g_nLabelFlag = 0;
static unsigned int Toggle = 2;
...
void drop_bits(register unsigned int num) {
    ...
    if (temp <= stream.num) {
        stream.pos = temp;
        bits -= num;
        stream.data = g_pMp2AudioFrame[temp - 1] << (32 - bits);
        stream.bits = bits;
        return;
    }
...
unsigned int read_bits(unsigned int num) {
    ...
    if (bits < num) {
        ...
        while ((bits <= 24) && (temp < stream.num)) {
            data += (g_pMp2AudioFrame[temp] << (24 - bits));
            bits += 8;
            temp++;
        }
        stream.pos = temp;
...
int audio_data_decode(int nch, int &sampling, int &PCM_Length, int bound, int sblimit, int table_id,
    ...
    if (CRC_check(0, CRC_bits[0])) {
        sN_ch = nch;
        sSampling = sampling;
        sPCM_Length = PCM_Length = 1152 * nch;
    } else {
        nch = sN_ch;
        sampling = sSampling;
...
//			for (sb=0;sb<4;sb++)
//				for (gr=0;gr<3;gr++)
//					sScaleFactor[ch][sb][gr] = ScaleFactor[ch][sb][gr];
//	}
...
void ancillary_data_decode(int Frame_Length, int table_id) {
    ...
    memcpy(sCRC_words.crc_word, &g_pMp2AudioFrame[Frame_Length - 6], 4);

    sCRC_words.flag = 1;

    memset(&f_pad, 0, sizeof(f_pad));

    temp = g_pMp2AudioFrame[Frame_Length - 2];
    ...
    temp = g_pMp2AudioFrame[Frame_Length - 1];
    f_pad.ByteL_data_field = (temp >> 2) & 0x3f;
    f_pad.CI = (temp >> 1) & 0x01;


    DAB_ReadXpad(g_pXpad, Frame_Length, table_id);
...
int mpeg_dec(int &nch, int &sampling, int &PCM_Length, short *PCM_Frame, int Frame_Length,
    ...
    audio_buf_init(Audio_Frame, Frame_Length);
    CRC_buf_init(Audio_Frame, Frame_Length);
    ...
    if (audio_data_decode(nch, sampling, PCM_Length, bound, sblimit, table_id, PCM_Frame) == 0) {
        return 0;
    }

    ancillary_data_decode(Frame_Length, table_id);
    Label_flag = g_nLabelFlag;
    Str_Length = Label.num;
    if (g_nLabelFlag) {
        memcpy(Str, Label.Str, Label.num);
        g_nLabelFlag = 0;
    }
    return 1;
}
...
//
//   ������XPAD�Ͷ�̬������ʾ�Ĵ��롣
//       ������short XPAD��variable XPAD�����
//   zhs080804
//
//------------------------------------------------------------------------------


const unsigned short gXpadCrcTable[256] = {
        0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7,
        0x8108, 0x9129, 0xA14A, 0xB16B, 0xC18C, 0xD1AD, 0xE1CE, 0xF1EF,
        0x1231, 0x0210, 0x3273, 0x2252, 0x52B5, 0x4294, 0x72F7, 0x62D6,
        0x9339, 0x8318, 0xB37B, 0xA35A, 0xD3BD, 0xC39C, 0xF3FF, 0xE3DE,
...
//-------------------------------------------
// Function: DAB_XpadInit(XPADPS_SETTING psXpad)
// Input: psXpad, points to XPAD setting.
// Return:
// Description:
//   Initialize the variables of XPAD decoding
//
//-------------------------------------------
void DAB_XpadInit(XPADPS_SETTING psXpad) {
    ...
    for (i = 0; i < 20; i++)
        psXpad->pSegDataGroup[i] = 0x00;

    for (i = 0; i < 136; i++)
        psXpad->pSegBuf[i] = 0x00;

    for (i = 0; i < 144; i++)
        psXpad->pDispBuf0[i] = '\0';

    for (i = 0; i < 16; i++)
        psXpad->pDispBuf1[i] = ' ';
    for (i = 16; i < 144; i++)
        psXpad->pDispBuf1[i] = '\0';

    psXpad->VXpadDLabelStartByteNo = 0;
    psXpad->VXpadSubFieldLength = 0;
    psXpad->ToggleFlag = 0xFF;
    psXpad->ToggleChangeFlag = 0;
    psXpad->FastenDispFlag = 0;
    psXpad->Language = 0;
    psXpad->SegNum = 0;
    psXpad->DispBufAddr = 0;
    psXpad->DataGroupByteNo = 0;
    psXpad->DispBufId = 0;
    psXpad->DispBuf0ByteNum = 16;
    psXpad->DispBuf1ByteNum = 0;
}


//-------------------------------------------
// Function: DAB_DlabelCrc(UINT8 * pStr, UINT8 ByteNum)
// Input:  pStr, points to the start address of Label segment
//         iLen, the length (in Byte) of Dynamic Label, includes
//              prefix and the segment byte
// Return: the CRC check result
// Description:
//   CRC check of the Dynamic Label
//
//-------------------------------------------
uint8_t DAB_DlabelCrc(uint8_t *pStr, uint8_t ByteNum) {
    ...
    for (i = 0; i < ByteNum; i++) {
        data = pStr[i];
        crc = (crc << 8) ^ gXpadCrcTable[((crc >> 8) ^ data) & 0xFF];
    }

    crc = crc & 0xFFFF;
    crc = crc ^ 0xFFFF;

    crc_word = (pStr[ByteNum] << 8) + pStr[ByteNum + 1];
...
//-------------------------------------------
// Function: DAB_DlabelSegDec(XPADPS_SETTING psXpad)
// Input: psXpad, points to XPAD setting.
// Return:
// Description:
//   decode a segment of Dlabel
//
//-------------------------------------------
void DAB_DlabelSegDec(XPADPS_SETTING psXpad) {
    uint8_t FirstLastFlag;
    uint8_t SegNo = 0;
    uint8_t SegLen = 0;
    uint8_t i;

    if ((psXpad->pSegDataGroup[0] & 0x80) != psXpad->ToggleFlag)  // toggle changed
    {
        if (psXpad->ToggleFlag != 0xFF)
            psXpad->ToggleFlag = (psXpad->pSegDataGroup[0] & 0x80);
    }

    // if current segment is a command, then quit
    if ((psXpad->pSegDataGroup[0]) & 0x10)
        return;

    // if CRC failure, then quit
    SegLen = psXpad->pSegDataGroup[0] & 0x0F;
    if (!DAB_DlabelCrc(psXpad->pSegDataGroup, SegLen + 3))
        return;
    else {
        FirstLastFlag = (psXpad->pSegDataGroup[0] & 0x60);
        if (FirstLastFlag & 0x40)                  // the first Segment
        {
            psXpad->Language = (psXpad->pSegDataGroup[1] & 0xF0) >> 4;
        } else
            SegNo = (psXpad->pSegDataGroup[1] & 0xF0) >> 4;
    }

    // copy to segment buffer
    *(psXpad->pSegBuf + 17 * SegNo) = SegLen;
    for (i = 1; i <= (SegLen + 1); i++) {
        *(psXpad->pSegBuf + 17 * SegNo + i) = psXpad->pSegDataGroup[1 + i];
    }

    if (FirstLastFlag & 0x20)                  // the last Segment
    {
        psXpad->SegNum = SegNo + 1;
        // the first time to display XPAD, display some welcom word
        if (psXpad->ToggleFlag <= 0xFD)
            DAB_UpdateXpadDispBuf(psXpad);
        else
            psXpad->ToggleFlag--;
    }
}
...
//-------------------------------------------
// Function: DAB_UpdateXpadDispBuf(XPADPS_SETTING psXpad)
// Input: psXpad, points to XPAD setting.
// Return:
// Description:
//   copy the current Dlabel to display buffer
//
//-------------------------------------------
void DAB_UpdateXpadDispBuf(XPADPS_SETTING psXpad) {
    ...
    uint8_t StartAddr;
    uint8_t *pDispBuf;
    uint8_t SegCrcErrFlag = 0;

    // first, Judge if there is any segment CRC error
    // Only all segment CRC is correct will do the update
    for (i = 0; i < psXpad->SegNum; i++)   // Max 8 segment
    {
        j = *(psXpad->pSegBuf + 17 * i);
        if (j & 0x80)      // if there is one segment CRC error
            SegCrcErrFlag = 1;
    }

    // reset the CRC error Flag of each segment
    for (i = 0; i < 8; i++)
        *(psXpad->pSegBuf + 17 * i) |= 0x80;

    if (SegCrcErrFlag)   // if there are segment CRC error, then do not update
        return;

    // clear the buffer area for new data
    if (psXpad->DispBufId == 0x01) {
        pDispBuf = psXpad->pDispBuf0;
        psXpad->DispBuf0ByteNum = 0;
    } else {
        if (psXpad->ToggleFlag <= 0xFD) {
            pDispBuf = psXpad->pDispBuf1;
            psXpad->DispBuf1ByteNum = 0;
        } else
            return;
    }
    ...
    StartAddr = 16;
    // copy the Dlabel to display buffer
    for (i = 0; i < psXpad->SegNum; i++)   // total 8 segment
    {
        SegLen = *(psXpad->pSegBuf + 17 * i);
        SegLen &= 0x7F;
        for (j = 1; j <= (SegLen + 1); j++)        // copy each segment
        {
            *(pDispBuf + StartAddr) = *(psXpad->pSegBuf + 17 * i + j);
            StartAddr++;
        }
    }

    if (psXpad->DispBufId == 0x01)
        psXpad->DispBuf0ByteNum = StartAddr;
    else
        psXpad->DispBuf1ByteNum = StartAddr;


    // ��XPAD��ʾ�����е����ݴ��ݳ�ȥ
    Label.num = StartAddr - 16;
    memcpy(Label.Str, pDispBuf + 16, Label.num);

    if (memcmp(psXpad->pDispBuf0 + 16, psXpad->pDispBuf1 + 16, Label.num)) {
        g_nLabelFlag = 1;
        if (psXpad->DispBufId == 0x01)
            psXpad->DispBufId = 0x00;
        else
            psXpad->DispBufId = 0x01;
    }
}


//-------------------------------------------
// Function: DAB_GetXpadSubFieldLength(UINT8 ContentIndicator)
// Input: ContentIndicator, the content indicator of the variable XPAD
// Return: the munber of bytes of the Xpad sub-field
// Description:
//   get the munber of bytes of the Xpad sub-field
//
//-------------------------------------------
uint8_t DAB_GetXpadSubFieldLength(uint8_t ContentIndicator) {
    uint8_t ByteNum;

    switch (ContentIndicator & 0xE0) {
        case 0x00:
            ByteNum = 4;
            break;
        case 0x20:
...
//-------------------------------------------
// Function: DAB_ReadXpad(XPADPS_SETTING psXpad)
// Input: psXpad, points to XPAD setting.
// Return:
// Description:
//   Read XPAD data from T-SRAM
//
//-------------------------------------------
void DAB_ReadXpad(XPADPS_SETTING psXpad, int32_t nFrameLen, int32_t nTableId) {
    int8_t ScfcrcByteNum;
    int8_t XpadInd;
    int8_t CI;
    int16_t BbReadAddr;
    int8_t i;
    ...
    // read F-PAD
    XpadInd = f_pad.X_PAD_Ind;
    CI = f_pad.CI;

    BbReadAddr = nFrameLen - 1;
    ...
    if (XpadInd == 0x01)     // short XPAD
    {
        psXpad->pXpadBuf[0] = g_pMp2AudioFrame[BbReadAddr--];
        psXpad->pXpadBuf[1] = g_pMp2AudioFrame[BbReadAddr--];
        psXpad->pXpadBuf[2] = g_pMp2AudioFrame[BbReadAddr--];
        psXpad->pXpadBuf[3] = g_pMp2AudioFrame[BbReadAddr--];

        if (CI)                  // the start of a XPAD data group
        {
            if (psXpad->pXpadBuf[0] == 2)  // the start of a DLabel data group
            {
                // if s new label starts, then copy the last label to display buffer
                DAB_DlabelSegDec(psXpad);

                psXpad->DataGroupByteNo = 0;
                psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[1];
                psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[2];
                psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[3];
            } else if (psXpad->pXpadBuf[0] == 3)  // the start of a DLabel data group
            {
                psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[1];
                psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[2];
                psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[3];
            ...
            psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[0];
            psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[1];
            psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[2];
            psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[3];
        }

        // to avoid the overflow of 'pSegDataGroup'
        if (psXpad->DataGroupByteNo >= 20)
            psXpad->DataGroupByteNo = 20;
    } else if (XpadInd == 0x02) // variable XPAD
    {
        // read 64 bytes
        for (i = 0; i < 64;) {
            psXpad->pXpadBuf[i++] = g_pMp2AudioFrame[BbReadAddr--];
        }

        if (CI) {
            // Detect if there is D Label sub-field
            // if there is no D Label sub-field, then quit
            for (i = 0; i < 4; i++)
                if ((psXpad->pXpadBuf[i] & 0x1E) == 0x02)
                    break;
            if (i == 4)
                return;

            // re-detect and get the total CI number
            psXpad->CINum = 0;
            for (i = 0; i < 4; i++) {
                if ((psXpad->pXpadBuf[i]) != 0x00)
                    psXpad->CINum++;
                else
                    break;
            }

            // re-detect and get the basic information of D Label sub-field
            if (psXpad->CINum < 4)
                psXpad->VXpadDLabelStartByteNo = psXpad->CINum + 1;
            else
                psXpad->VXpadDLabelStartByteNo = 4;

            for (i = 0; i < psXpad->CINum; i++) {
                psXpad->VXpadSubFieldLength = DAB_GetXpadSubFieldLength(psXpad->pXpadBuf[i]);

                if ((psXpad->pXpadBuf[i] & 0x1E) == 0x02) {
                    // if s new label starts, then copy the last label to display buffer
                    if ((psXpad->pXpadBuf[i] & 0x1F) == 0x02) {
                        DAB_DlabelSegDec(psXpad);
                        psXpad->DataGroupByteNo = 0;
                    }

                    // if exceed the decoder capacity, then quit
                    if ((psXpad->VXpadDLabelStartByteNo + psXpad->VXpadSubFieldLength) >= 64)
                        return;

                    for (j = 0; j < psXpad->VXpadSubFieldLength; j++) {
                        psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[
                                psXpad->VXpadDLabelStartByteNo + j];
                        if (psXpad->DataGroupByteNo >= 22)
                            psXpad->DataGroupByteNo = 22;
                    }
                }

                psXpad->VXpadDLabelStartByteNo += psXpad->VXpadSubFieldLength;
            }
        } else   // no content identifier
        {
            for (i = 0; i < psXpad->VXpadSubFieldLength; i++) {
                psXpad->pSegDataGroup[psXpad->DataGroupByteNo++] = psXpad->pXpadBuf[
                        psXpad->VXpadDLabelStartByteNo + i];
                if (psXpad->DataGroupByteNo > 22)
                    psXpad->DataGroupByteNo = 22;
            }
        }
